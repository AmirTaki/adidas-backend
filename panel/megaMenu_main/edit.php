<?php
    ...
    $tableHeader  =  readTable ("adidas", "SELECT * FROM adidas.menu_main", $single = false, $execute = null);
    if ($_GET['id']!== "" and isset($_GET['id'])){
    
        $TableMegaMenu =  readTable ("adidas", "SELECT * FROM adidas.megamenu_main WHERE id = ?", $single = true, $execute = [$_GET['id']]);
      
        if($TableMegaMenu){
            if(isset($_POST['category']) and $_POST['category']!== ""  and isset($_POST['title']) and $_POST['title'] !== "" )
            {
               $operation =  operationsDataBase ("adidas", "UPDATE adidas.megamenu_main SET category = ?, title = ?,  updated_at = NOW() WHERE id = ?", $execute = [$_POST['category'], $_POST['title'], $_GET['id']]);
               $operation ? redirect("panel/megaMenu_main"): "";
            }
            else {
...
<body>
    ...
    <div style = 'padding-top:150px'>
        <form action="<?= url('panel/megaMenu_main/edit.php?id='.$_GET['id']); ?>" method="post" class = 'form_create_category' enctype="multipart/form-data">
            <!-- Category  -->
